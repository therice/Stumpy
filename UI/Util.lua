...
function U.ToShortKey(key)
    ...
    if key then
        key = key:upper()
        key = key:gsub(' ', '')
        key = key:gsub('SHIFT%-', L["KEY_SHIFT"])
        key = key:gsub('ALT%-', L["KEY_ALT"])
        key = key:gsub('CTRL%-', L["KEY_CTRL"])
        key = key:gsub('BUTTON', L["KEY_MOUSEBUTTON"])
        key = key:gsub('MOUSEWHEELUP', L["KEY_MOUSEWHEELUP"])
        key = key:gsub('MOUSEWHEELDOWN', L["KEY_MOUSEWHEELDOWN"])
        key = key:gsub('NUMPAD', L["KEY_NUMPAD"])
        key = key:gsub('PAGEUP', L["KEY_PAGEUP"])
        key = key:gsub('PAGEDOWN', L["KEY_PAGEDOWN"])
        key = key:gsub('SPACE', L["KEY_SPACE"])
        key = key:gsub('INSERT', L["KEY_INSERT"])
        key = key:gsub('HOME', L["KEY_HOME"])
        key = key:gsub('DELETE', L["KEY_DELETE"])
        key = key:gsub('NMULTIPLY', '*')
        key = key:gsub('NMINUS', 'N-')
        key = key:gsub('NPLUS', 'N+')
        key = key:gsub('NEQUALS', 'N=')
        --Logging:Debug("ToShortKey() : %s", key)
        return key
    end
end
